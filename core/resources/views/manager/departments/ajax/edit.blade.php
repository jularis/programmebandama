    ...
    <div class="col-sm-12">
        <x-form id="save-department-data-form" method="PUT">
            <div class="add-client bg-white rounded">
                ...
                <div class="row p-20">
                    <div class="col-md-6">
                        <x-forms.text fieldId="department" :fieldLabel="__('app.name')" fieldName="department"
                            fieldRequired="true" fieldValue="{{ $department->department }}">
                        </x-forms.text>
                    </div>
                    <div class="col-md-6">
                        ...
                        <x-forms.input-group>
                            <select class="form-control select-picker mt" name="parent_id" id="parent_id"
                                ...
                                @foreach($departments as $item)
                                        <option value="{{ $item->id }}" @if($department->parent_id == $item->id) selected @endif>{{ $item->department }}</option>
                                @endforeach
                            </select>
                        </x-forms.input-group>
                    </div>
                ...
                <x-form-actions>
                    ...
                    <x-forms.button-cancel :link="route('manager.departments.index')" class="border-0">@lang('app.cancel')
...
<script>
    $(document).ready(function() {
        ...
        $('#save-department-form').click(function() {
            ...
            const url = "{{ route('manager.departments.update', $department->id) }}";
