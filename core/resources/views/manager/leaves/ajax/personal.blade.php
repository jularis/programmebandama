<div class="d-grid d-lg-flex d-md-flex action-bar">
    ...
    <div class="btn-group mt-2 mt-lg-0 mt-md-0 ml-3" role="group" aria-label="Basic example">
        ...
        <a href="{{ route('manager.leaves.personal') }}" class="btn btn-secondary f-14 btn-active" data-toggle="tooltip"
                ...
                @if ($employee->status == 'active')
                    <p class="card-text f-12 text-lightest">@lang('app.lastLogin')
                        ...
                        @if (!is_null($employee->last_login))
                            {{ $employee->last_login->timezone('Africa/Abidjan')->translatedFormat('Y-m-d H:i') }}
                        @else
                            --
                        @endif
                    </p>
...
<script>
    $(document).ready(function() {
        ...
        $('.update-category').click(function() {
            ...
            var url = "{{ route('manager.employee-leaves.update', ':id') }}";
