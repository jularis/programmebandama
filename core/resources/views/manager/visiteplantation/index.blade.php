    ...
    <div class="row mb-none-30">
        <div class="col-lg-12 mb-30">
                ...
                <div class="card-body">
                    <form action="">
                        <div class="d-flex flex-wrap gap-4">
                            <input type="hidden" name="table" value="visiteplantation"/>
            ...
            <div class="card b-radius--10 ">
                <div class="card-body  p-0">
                    <div class="table-responsive--sm table-responsive">
                        <table class="table table--light style--two">
                            ...
                            <tbody>
                                @forelse($visitePlantations as $data)
                                    <tr>
                                        <td>
                                        ...
                                        <td>
                                            ...
                                            <div class="dropdown-menu p-0">
                                                <a href="{{ route('manager.suivi.visiteplantation.edit', $data->id) }}"
                                                    class="dropdown-item"><i class="la la-pen"></i>@lang('Editer')</a>
                                                <a href="{{ route('manager.suivi.visiteplantation.show', $data->id) }}"
                                                    class="dropdown-item"><i class="las la-file-invoice"></i>@lang('Détail')</a>
                                                @if ($data->status == Status::DISABLE)
                                                    <button type="button" class="confirmationBtn  dropdown-item"
                                                        data-action="{{ route('manager.suivi.visiteplantation.status', $data->id) }}"
                                                        data-question="@lang('Are you sure to enable this visite de plantation?')">
                                                        <i class="la la-eye"></i> @lang('Active')
                                                    </button>
                                                @else
                                                    <button type="button" class="confirmationBtn dropdown-item"
                                                        data-action="{{ route('manager.suivi.visiteplantation.status', $data->id) }}"
                                                        data-question="@lang('Are you sure to disable this visite de plantation?')">
                                                        <i class="la la-eye-slash"></i> @lang('Désactive')
                                                    </button>
                                                @endif
                                                <button type="button" class="confirmationBtn dropdown-item"
                                                    data-action="{{ route('manager.suivi.visiteplantation.delete', encrypt($data->id)) }}"
                                                    data-question="@lang('Are you sure to delete this visite de plantation?')">
                ...
                @if($visitePlantations->hasPages())
                    <div class="card-footer py-4">
                        {{ paginateLinks($visitePlantations) }}
                    </div>
                @endif
            </div>
        </div>
...
@push('breadcrumb-plugins')
    <a href="{{ route('manager.suivi.visiteplantation.create') }}" class="btn  btn-outline--primary h-45 addNewCooperative">
        <i class="las la-plus"></i>@lang("Ajouter nouveau")
    </a>
    <a href="{{ route('manager.suivi.visiteplantation.exportExcel.plantationAll') }}" class="btn  btn-outline--warning h-45"><i class="las la-cloud-download-alt"></i> @lang('Exporter en Excel')</a>
@endpush
