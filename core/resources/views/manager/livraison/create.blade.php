    ...
    (function ($) {
        ...
        $('.addUserData').on('click', function () {
            ...
            <div class="row single-item gy-2">
                <div class="col-md-3">
                    <select class="form-control selected_type select2-basic" data-live-search="true"  name="items[${length}][producteur]" required id='producteur-${length}' onchange=getParcelle(${length})>
                        <option disabled selected value="">@lang('Producteur')</option>
                        @foreach($producteurs as $producteur)
                            <option value="{{$producteur->id}}" data-id="{{$producteur->id}}" data-price="{{ $periode->prix_champ}} ?? 0 ">{{__(stripslashes($producteur->nom))}} {{__(stripslashes($producteur->prenoms))}}({{__(stripslashes($producteur->codeProd))}})</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-4">
