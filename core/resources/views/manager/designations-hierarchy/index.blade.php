    ...
    <div class="content-wrapper">
        ...
        <div class="d-grid d-lg-flex d-md-flex action-bar">
            <div id="table-actions" class="flex-grow-1 align-items-center">
                ...
                <x-forms.link-primary :link="route('manager.designations.create')" class="mr-3 openRightModal float-left" icon="plus" data-redirect-url="{{ route('manager.designation.hierarchy') }}">
            ...
            <div class="btn-group mt-2 mt-lg-0 mt-md-0 ml-0 ml-lg-3 ml-md-3" role="group">
                <a href="{{ route('manager.designations.index') }}" class="btn btn-secondary f-14" data-toggle="tooltip"
                data-original-title="@lang('modules.leaves.tableView')"><i class="side-icon bi bi-list-ul"></i></a>

                <a href="{{ route('manager.designation.hierarchy') }}" class="btn btn-secondary f-14 btn-active" data-toggle="tooltip"
            ...
            <div class="col-12 chart-section">
                <div class="row">
                    <div class="col-md-6" id="chartTree">
                        @include('manager.designations-hierarchy.chart_tree')
                    </div>

                    <div class="col-md-6" id="chartOrganization">
                        @include('manager.designations-hierarchy.chart_organization')
                    </div>
                </div>
            </div>
        </div>
...
$('#search-text-field').on('change keyup',
 ...
 function searchText(){
        ...
        $.ajax({
            url: "{{ route('manager.designation.srchFilter') }}",
            type: "POST",
            data: {
                "_token": "{{ csrf_token() }}",
                "searchText": srchText
                ...
                if (parent_id == 'NewNode') {
                    var checkExits = setInterval(() => {
                        if ($('#node-ul-NewNode').length) {
                            ...
                            $.ajax({
                                url: "{{ route('manager.designation.changeParent') }}",
                                type: "POST",
                                data: {
                                    "_token": "{{ csrf_token() }}",
                                    "newParent": $newParent,
