    ...
    <div class="content-wrapper">
        ...
        <div class="d-grid d-lg-flex d-md-flex action-bar">
            <div id="table-actions" class="flex-grow-1 align-items-center">
                ...
                <x-forms.link-primary :link="route('manager.departments.create')" class="mr-3 openRightModal float-left" icon="plus" data-redirect-url="{{ route('manager.department.hierarchy') }}">
            ...
            <div class="btn-group mt-2 mt-lg-0 mt-md-0 ml-0 ml-lg-3 ml-md-3" role="group">
                <a href="{{ route('manager.departments.index') }}" class="btn btn-secondary f-14" data-toggle="tooltip"
                data-original-title="@lang('modules.leaves.tableView')"><i class="side-icon bi bi-list-ul"></i></a>

                <a href="{{ route('manager.department.hierarchy') }}" class="btn btn-secondary f-14 btn-active" data-toggle="tooltip"
            ...
            <div class="col-12 chart-section">
                <div class="row">
                    <div class="col-md-6" id="chartTree">
                        @include('manager.departments-hierarchy.chart_tree')
                    </div>
                    <div class="col-md-6" id="chartOrganization">
                        @include('manager.departments-hierarchy.chart_organization')
                    </div>
                </div>
            </div>
        </div>
...
<script>
            ...
            if (parent_id == 'NewNode') {
                var checkExits = setInterval(() => {
                    if ($('#node-ul-NewNode').length) {
                        ...
                        $.ajax({
                            url: "{{ route('manager.department.changeParent') }}",
                            type: "POST",
                            data: {
                                "_token": "{{ csrf_token() }}",
                                "newParent": $newParent,
    ...
    function searchText() {
        ...
        const url = "{{ route('manager.departments.search') }}";
