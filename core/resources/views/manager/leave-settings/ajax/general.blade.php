    ...
    <div class="d-block d-lg-flex d-md-flex">
        ...
        <div class="col-lg-4">
            ...
            <select name="permission" class="form-control select-picker manager-permission"
                ...
                <option value="pre-approve">@lang('modules.leaves.preApprove')</option>
                <option value="approved">@lang('modules.leaves.approve')</option>
                <option value="cannot-approve">@lang('modules.leaves.canNotApprove')</option>
            </select>
        </div>
...
<script>
    ...
    $('input[name=leaves_start_from], #year_starts_from').on("click change", function () {
        ...
        $.easyAjax({
            url: "{{ route('manager.settings.leaves-settings.store') }}",
            type: "POST",
            data: {
                '_token': '{{ csrf_token() }}',
    ...
    function changeStatus(value) {
        ...
        var url = "{{ route('manager.settings.leaves-settings.changePermission') }}";
