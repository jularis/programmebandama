...
@section('panel')
                                    ...
                                    @else
                                        @if($k != 'images')
                                            ...
                                            @elseif($k == 'select')
                                                ...
                                                <div class="col-md-12">
                                                    <div class="form-group">
                                                        <label>{{__(keyToTitle(@$selectNom))}}</label>
                                                        <select class="form-control" name="{{ @$selectNom }}">
                                                            @foreach($item->options as $selectItemKey => $selectOption)
                                                                <option value="{{ $selectItemKey }}" @if(@$content->data_values->$selectNom == $selectItemKey) selected @endif>{{ $selectOption }}</option>
                                                            @endforeach
                                                        </select>
                                                    </div>
                                                </div>
    ...
    @if(@$section->element)
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="button--group">
                                                ...
                                                data-question="@lang('Êtes-vous sûr de vouloir supprimer ce item?')"><i class="la la-trash"></i> @lang('Remove')</button>
                                            </div>
                                        </td>
                                    </tr>
                                @empty
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title"> @lang("Ajouter nouveau") {{__(keyToTitle($key))}} @lang('Item')</h5>
        ...
        <div class="d-flex flex-wrap justify-content-end gap-2 align-items-center">
            @if($section->element->modal)
                <a href="javascript:void(0)" class="btn btn-sm btn-outline--primary addBtn"><i class="las la-plus"></i>@lang("Ajouter nouveau")</a>
            @else
                <a href="{{route('admin.frontend.sections.element',$key)}}" class="btn btn-sm btn-outline--primary"><i class="las la-plus"></i>@lang("Ajouter nouveau")</a>
            @endif
        </div>
        @endpush
    @endif
...
@push('style-lib')
<link href="{{ asset('assets/fcadmin/css/fontawesome-iconpicker.min.css') }}" rel="stylesheet">
@endpush

@push('script-lib')
    <script src="{{ asset('assets/fcadmin/js/fontawesome-iconpicker.js') }}"></script>
@endpush
