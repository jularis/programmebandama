                        ...
                        <div class="text-center">
                            @php
                                ...
                                foreach ($livraisonInfo->products as $prodc) {
                                    $numeroProducteurs .= @$prodc->producteur->nom.' '.@$prodc->producteur->prenoms.'('.@$prodc->producteur->codeProdapp.')'."\n";
                                }
                                ...
                                <tbody>
                                    @foreach ($livraisonInfo->products as $livraisonProductInfo)
                                        <tr>
                                            ...
                                            <td>{{ __(@$livraisonProductInfo->producteur->programme->libelle) }}</td>
                                            <td>{{ __(@$livraisonProductInfo->producteur->nom) }} {{ __(@$livraisonProductInfo->producteur->prenoms) }}</td>
                                            <td>{{ __(@$livraisonProductInfo->parcelle->codeParc) }}</td>
                                            <td>{{ __(@$livraisonProductInfo->certificat) }}</td>
                                            <td>{{ __(@$livraisonProductInfo->type_produit) }}</td>
                                            <td>{{ showAmount($livraisonProductInfo->quantite) }} Kg</td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
                            ...
                            <div class="table-responsive">
                                <table class="table">
                                    <tbody>
                                        <tr>
                                            <th>@lang('Total'):</th>
                                            <td>{{ showAmount(@$livraisonInfo->products->sum('quantite')) }} Kg</td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
            ...
            <div class="row no-print">
                <div class="col-sm-12">
                    <div class="float-sm-end">
                        <button class="btn btn-outline--primary printFacture"><i class="las la-download"></i>@lang('Imprimer')</button>
                    </div>
                </div>
            </div>
        </div>
